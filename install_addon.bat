...
"$launch = '%LAUNCH_PATH:\\=\\\\%';" ^ "$json = ConvertTo-Json @{version=1; commands=@(@{id='com.tools.bus-routing-auditor'; displayName='Bus Routing Auditor'; program=$launch; mainMenu=@{basePath='Tools'}})} -Depth 5;" ^ "Set-Content -Path '%ADDON_DIR%\\BusRoutingAuditor.json' -Value $json -Encoding UTF8"
if errorlevel 1 (

echo \[오류\] Add-on 파일 생성 실패 pause exit /b 1
)
